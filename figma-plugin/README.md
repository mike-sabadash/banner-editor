...
6. Select the animated source layer and choose **Sync motion to family** or **Sync motion everywhere**. Translation tracks are copied relative to each target layer's local position; timing, easing, opacity, scale and rotation are preserved.
7. Export `.figma-campaign.json` and open it through **Open** in Banner Editor.
