...
async function creativePayload(campaignId){const result=[];for(const root of campaignRoots(campaignId)){let previewSvg="";try{previewSvg=await root.exportAsync({format:"SVG_STRING",svgOutlineText:false,svgIdAttribute:true})}catch{}result.push({formatId:getMeta(root,"formatId"),previewType:previewSvg?"figma-svg":"figma",previewSvg,durationSec:Number(getMeta(root,"duration")||getMeta(root,"durationSec")||6)})}return result.filter(x=>x.formatId)}
