...
const ui=readFileSync(new URL("./ui-v14.html",import.meta.url),"utf8");
...
describe("Figma campaign + TT Knowledge contract",()=>{
  it("parses UI/controller without syntax errors",()=>{const script=ui.match(/<script>([\s\S]*?)<\/script>/)?.[1]||"";expect(script.length).toBeGreaterThan(100);expect(()=>compile(script,"ui-v14-inline.js")).not.toThrow();expect(()=>compile(controller,"code-v8.js")).not.toThrow()});
  ...
  it("keeps campaign setup, knowledge and creative workspaces",()=>{for(const id of ["setup","knowledge","creative","manual","plan"])expect(ui).toContain(`id=\"${id}\"`)});
  it("keeps tri-state TT requirements",()=>{for(const x of ["required","not required","function fromTri(v)"])expect(ui).toContain(x)});
  it("adds a dedicated TT Knowledge workspace",()=>{for(const x of ["TT Knowledge","TT Knowledge Base · 2026","Search platform, publisher, format","All statuses","Refresh"])expect(ui).toContain(x)});
  it("shows source provenance and refresh metadata",()=>{for(const x of ["sourceUrl","sourceTitle","lastCheckedAt","sourceType","refreshMode"])expect(kb).toContain(x);expect(ui).toContain("checked ${esc(i.lastCheckedAt||'never')}");expect(ui).toContain("sourceTitle")});
  ...
  it("lets user explicitly select KB records for AI",()=>{expect(ui).toContain("kbSelected=new Set()");expect(ui).toContain("data-kb-select");expect(ui).toContain("knowledgeIds:[...kbSelected]");expect(gateway).toContain("kbItemsAsDocuments(input.knowledgeIds||[])")});
  it("uses uploaded TT files or selected KB sources for explicit AI match/verify",()=>{expect(ui).toContain("Attach TT files or select TT Knowledge sources");expect(ui).toContain("Find TT with AI");expect(ui).toContain("Verify current setup with AI");expect(gateway).toContain('req.url==="/api/tt/analyze"')});
  it("keeps media plan and manual campaign creation",()=>{for(const x of ["Manual setup","Media plan / TT","Create campaign","Create formats from plan","function rowsToPlacements","function buildFormats(placements)"])expect(ui).toContain(x)});
