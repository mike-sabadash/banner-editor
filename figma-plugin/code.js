...
async function syncMotion(scope){const source=figma.currentPage.selection[0];if(!source)return{count:0,error:"Select one animated source layer"};const slotId=getMeta(source,"slotId");if(!slotId)return{count:0,error:"Assign a campaign slot first"};const sourceRoot=formatRoot(source);if(!sourceRoot)return{count:0,error:"The source layer is not inside a campaign format"};const tracks=source.manualKeyframeTracks||{},available=MOTION_FIELDS.filter(field=>tracks[field]);if(!available.length)return{count:0,error:"The selected layer has no supported manual Motion tracks"};const campaignId=getMeta(sourceRoot,"campaignId"),familyId=getMeta(sourceRoot,"familyId");let count=0;
  ...
  return{count,tracks:available.length,scope};
}
