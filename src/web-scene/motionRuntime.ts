...
export function motionFrame(layer:MotionLayer,localMs:number):CSSProperties{
 ...
 else if(["from-left","from-right","from-top","from-bottom"].includes(preset)){const vector=isOut?(layer.outMotionVector??layer.motionVector):layer.motionVector,factor=isOut?p:inverse;transform=`translate3d(${vector.x*factor}%,${vector.y*factor}%,0) scale(1)`}
