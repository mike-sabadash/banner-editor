...
export function motionFrame(layer:MotionLayer,localMs:number):CSSProperties{
 ...
 else if(preset==="bounce"){const amp=layer.bounceIntensity??14,bounces=layer.bounceBounces??3,envelope=Math.pow(1-p,1.35),d=Math.abs(Math.sin(p*Math.PI*bounces))*amp*envelope,dir=layer.bounceDirection??"up",x=dir==="left"?-d:dir==="right"?d:0,y=dir==="up"?-d:dir==="down"?d:0;transform=`translate3d(${Number(x.toFixed(4))}%,${Number(y.toFixed(4))}%,0) scale(1)`}
