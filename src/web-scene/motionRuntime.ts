...
export const cubicBezierProgress=(t:number,curve:BezierCurve)=>{if(t<=0)return 0;if(t>=1)return 1;const[x1,y1,x2,y2]=curve,cx=3*x1,bx=3*(x2-x1)-cx,ax=1-cx,cy=3*y1,by=3*(y2-y1)-cy,ay=1-cy;const sampleX=(u:number)=>((ax*u+bx)*u+cx)*u;let lo=0,hi=1,u=t;for(let i=0;i<18;i++){u=(lo+hi)/2;const x=sampleX(u);if(x<t)lo=u;else hi=u}return clamp01(((ay*u+by)*u+cy)*u)};
