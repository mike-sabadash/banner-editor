...
export function motionFrame(layer:MotionLayer,localMs:number):CSSProperties{
 ...
 else if(preset==="bounce"){const amp=layer.bounceIntensity??14,bounce=clamp01(layer.bounceAmount??.25),curve=layer.bounceCurve??[.2,.8,.2,1],time=cubicBezierProgress(inRaw,curve),cycles=1.5+bounce*2.5,damping=3.2-bounce*1.8,envelope=Math.exp(-damping*time)*(1-time),d=Math.sin(time*Math.PI*cycles)*amp*envelope,dir=layer.bounceDirection??"up",x=dir==="left"?-d:dir==="right"?d:0,y=dir==="up"?-d:dir==="down"?d:0;transform=`translate3d(${Number(x.toFixed(4))}%,${Number(y.toFixed(4))}%,0) scale(1)`}
