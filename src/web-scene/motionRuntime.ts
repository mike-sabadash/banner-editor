...
export function motionFrame(layer:MotionLayer,localMs:number):CSSProperties{
 ...
 else if(["from-left","from-right","from-top","from-bottom"].includes(preset)){const vector=isOut?(layer.outMotionVector??layer.motionVector):layer.motionVector,x=Number((vector.x*inverse).toFixed(4)),y=Number((vector.y*inverse).toFixed(4));transform=`translate3d(${x}%,${y}%,0) scale(1)`}
