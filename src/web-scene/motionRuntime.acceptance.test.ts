...
describe("motion runtime acceptance",()=>{
 it("combines IN transform and fade",()=>{const a=motionFrame(base,100),b=motionFrame(base,300),c=motionFrame(base,500);expect(a.opacity).toBe(0);expect(String(a.transform)).toContain("-100%");expect(b.opacity).toBeCloseTo(.5,3);expect(c.opacity).toBeCloseTo(1,6)});
 it("combines OUT transform and fade without a final jump",()=>{const a=motionFrame(base,1800),b=motionFrame(base,1950),c=motionFrame(base,2100);expect(a.opacity).toBeCloseTo(1,6);expect(b.opacity).toBeCloseTo(.5,3);expect(c.opacity).toBe(0);expect(String(c.transform)).toContain("-100%")});
