...
export const SPRING_PRESETS:Record<SpringPresetId,Omit<SpringConfig,"preset"|"direction">>={
...
export const springConfig=(value?:Partial<SpringConfig>):SpringConfig=>{const preset=value?.preset??"single",base=preset==="custom"?SPRING_PRESETS.single:SPRING_PRESETS[preset];return{preset,direction:value?.direction??"down",...base,...value,bounce:Math.max(-.5,Math.min(.5,value?.bounce??base.bounce)),velocity:Math.max(-10,Math.min(10,value?.velocity??base.velocity)),distance:Math.max(.05,Math.min(4,value?.distance??base.distance))}};
