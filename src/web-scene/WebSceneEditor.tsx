...
export default function WebSceneEditor(){
  ...
  const patchLayerById=(targetId:string,patch:Partial<Scene["layers"][number]>)=>setScenes(all=>all.map(s=>s.id===scene.id?{...s,layers:s.layers.map(l=>l.id===targetId?{...l,...patch}:l)}:s));
  const patchLayer=(patch:Partial<Scene["layers"][number]>)=>patchLayerById(layer.id,patch);
  const addScene=()=>{const stamp=Date.now(),n=scenes.length+1;const next:Scene={id:`scene-${stamp}`,name:`Scene ${n}`,durationMs:1800,layers:[{id:`bg-${stamp}`,name:"Background",role:"background",color:"#111827",motion:"none",durationMs:0,easing:"linear",visible:true},{id:`headline-${stamp}`,name:"Headline",role:"headline",text:"Новый экран",color:"#ffffff",motion:"fade",durationMs:350,easing:"ease-out",visible:true}]};setScenes(v=>[...v,next]);setSceneId(next.id);setLayerId(next.layers[1].id)};
