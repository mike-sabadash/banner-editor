...
export const editorActions={
  ...
  addAssetToCanvas(id:string,position?:{x:number;y:number}){const asset=state.assets.find((item)=>item.id===id);if(asset)this.addImage(asset.name,asset.assetUrl,position,{width:asset.width,height:asset.height})},
  ...
  addImage(name:string,url:string,position?:{x:number;y:number},natural?:{width:number;height:number}){const format=formats.find((f)=>f.id===state.activeFormat)!;const naturalWidth=natural?.width||format.width*.4,naturalHeight=natural?.height||naturalWidth*.65;const width=Math.max(1,Math.min(100,naturalWidth/format.width*100));const renderedHeight=naturalHeight/naturalWidth*(width/100*format.width)/format.height*100;const x=position?.x??Math.max(0,(100-width)/2),y=position?.y??Math.max(0,(100-renderedHeight)/2);const el:BannerElement={id:crypto.randomUUID(),kind:"image",name,text:"",assetUrl:url,x,y,width,scale:100,rotation:0,opacity:100,fontFamily:"Arial",fontSize:16,lineHeight:100,color:"#000000",inPoint:0,outPoint:state.duration,locked:false,visible:true,contentLinked:true};patch((s)=>{const elementsByFormat={...s.elementsByFormat,[s.activeFormat]:[...(s.elementsByFormat[s.activeFormat]??[]),el]};const base={elementsByFormat,selectedId:el.id,selectedIds:[el.id],formatOverrides:touchOverride(s)};return s.activeFormat==="master"?{...base,...adaptedState({...s,elementsByFormat} as ProjectState,false)}:base})},
  reorderElement(id:string,toIndex:number){patch((s)=>{const list=[...(s.elementsByFormat[s.activeFormat]??[])],from=list.findIndex((e)=>e.id===id);if(from<0)return{};const [item]=list.splice(from,1);list.splice(Math.max(0,Math.min(list.length,toIndex)),0,item);return{elementsByFormat:{...s.elementsByFormat,[s.activeFormat]:list},formatOverrides:touchOverride(s)}})},
  ...
  updateElement(id:string,values:Partial<BannerElement>,keyed=true){const target=elementList().find((e)=>e.id===id);if(!target)return;const anim=Object.entries(values).filter(([k])=>["x","y","scale","rotation","opacity"].includes(k)) as [AnimatableProperty,number][];const stat=Object.fromEntries(Object.entries(values).filter(([k])=>!["x","y","scale","rotation","opacity"].includes(k))) as Partial<BannerElement>;const layerFrames=frameMap()[id]??[],hasAnyKey=layerFrames.length>0,hasKey=layerFrames.some((f)=>Math.abs(f.time-state.playhead)<.035);
    ...
    if(Object.keys(stat).length||!keyed||!hasAnyKey)patch((s)=>{
      ...
      const elementsByFormat=Object.fromEntries(Object.entries(s.elementsByFormat).map(([formatId,list])=>[formatId,list.map((e)=>e.id===id?{
        ...
        ...(formatId===s.activeFormat&&(!keyed||!hasAnyKey)?Object.fromEntries(anim):{}),
      }:e)]));
      return{elementsByFormat,formatOverrides:touchOverride(s)};
    },{group});
    if(keyed&&anim.length&&hasAnyKey)this.upsertProperties(id,Object.fromEntries(anim),"inOutCubic",[.42,0,.58,1],group);
  },
