...
export const editorActions={
  ...
  moveKeyframeGroup(elementId:string,time:number,newTime:number){patch((s)=>{const list=s.keyframesByFormat[s.activeFormat]?.[elementId]??[];const clamped=Math.max(0,Math.min(s.duration,newTime));const keyframesByFormat={...s.keyframesByFormat,[s.activeFormat]:{...(s.keyframesByFormat[s.activeFormat]??{}),[elementId]:list.map((f)=>Math.abs(f.time-time)<.035?{...f,time:clamped}:f).sort((a,b)=>a.time-b.time)}};return{keyframesByFormat,playhead:clamped,formatOverrides:touchOverride(s)}})},
