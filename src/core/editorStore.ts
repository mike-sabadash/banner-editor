...
export const editorActions={
  ...
  select(id:string|null){patch({selectedId:id,selectedKeyframeId:null},{history:false})},
  ...
  selectKey(id:string,elementId:string){const frame=(frameMap()[elementId]??[]).find((f)=>f.id===id);if(frame)patch({selectedId:elementId,selectedKeyframeId:id,playhead:frame.time},{history:false})},
  ...
  importProject(project:ProjectState){remember();state={...fresh(),...project,version:2,formatOverrides:{...blankOverrides(),...(project.formatOverrides??{})}};emit()},
