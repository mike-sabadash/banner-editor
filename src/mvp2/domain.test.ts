...
import {assetQualityForCampaign,campaignReadiness,can,compileVisualFormats,densityState,effectiveSourceDensity,formatRequirementBadges,physicalExportSize,type AssetSource,type Campaign,type CreativeDocument,type FormatSpec,type Placement} from "./domain";
...
describe("Bannermatic MVP2 campaign compiler domain",()=>{
 ...
 it("computes canonical effective density cases",()=>{expect(effectiveSourceDensity({sourceWidth:400,sourceHeight:600},{width:200,height:300})).toBe(2);expect(effectiveSourceDensity({sourceWidth:300,sourceHeight:450},{width:200,height:300})).toBe(1.5);expect(effectiveSourceDensity({sourceWidth:200,sourceHeight:300},{width:200,height:300})).toBe(1)});
 it("classifies density against required export scale",()=>{expect(densityState(2,2)).toBe("ok");expect(densityState(1.6,2)).toBe("warning");expect(densityState(1,2)).toBe("insufficient");expect(densityState(1,1)).toBe("ok")});
 it("reports campaign raster quality without changing placement geometry",()=>{const format={...compileVisualFormats([{id:"p",platform:"x",placement:"x",width:200,height:300,requirements:{exportScale:2}}])[0]};const campaign:Campaign={id:"q",name:"Q",status:"creative",locale:"en",placements:[],formats:[format],creativeDocument:{version:2,masterFormat:{width:200,height:300},assets:{hero:{id:"hero",origin:"upload",sourceWidth:300,sourceHeight:450,storageUrl:"/hero.png",vector:false}},scenes:[{id:"s",layers:[{id:"l",name:"Hero",kind:"image",provenance:{assetId:"hero"},masterBox:{x:0,y:0,w:100,h:100}}]}]}};const q=assetQualityForCampaign(campaign)[0];expect(q).toMatchObject({sourceWidth:300,sourceHeight:450,placedWidth:200,placedHeight:300,density:1.5,requiredScale:2,state:"warning"});expect((campaign.creativeDocument!.scenes[0].layers[0] as any).masterBox).toEqual({x:0,y:0,w:100,h:100})});
