...
function CreativePreview({format,command,onOpen}:{format:VisualFormat;command:Command;onOpen:()=>void}){
 ...
 if(format.previewSvg)return <div className="ne-wall-preview" style={{aspectRatio:`${format.width}/${format.height}`}}><iframe srcDoc={svgDoc(format.previewSvg)} title={`${format.size} published Figma preview`} sandbox=""/><span className="ne-wall-preview-kind">Figma snapshot</span><button className="ne-wall-open" onClick={onOpen} aria-label="Open creative in Figma">↗</button></div>;
 return <button className="ne-wall-placeholder" style={{aspectRatio:`${format.width}/${format.height}`}} onClick={onOpen}><div><strong>BANNERMATIC</strong><b>{format.size}</b><span>Publish from Figma to replace this placeholder.</span></div></button>;
}
...
export default function CampaignWall({campaign,locale,onOpenFigma}:{campaign:Campaign;locale:Locale;onOpenFigma:()=>void}){
 ...
 return <div className="ne-workspace-page">
  ...
  <div className="ne-wall-grid">{campaign.formats.map(format=>{const ps=format.placementIds.map(placement).filter(Boolean) as Placement[];const previewLabel=isLive(format)?'Live HTML':format.previewSvg?'Figma snapshot':'Not published';const state=format.creativeState==='published'?'ready':format.creativeState==='draft'?'warning':'blocked';return <article className="ne-wall-card" key={format.id}><div className="ne-wall-card-head"><div><b>{format.size}</b><small>{ps.length} placement{ps.length===1?'':'s'}</small></div><span className={statusClass(state)}>{format.creativeState}</span></div><CreativePreview format={format} command={command} onOpen={onOpenFigma}/>{mode==='creative'?<div className="ne-wall-meta"><div><span>Creative version</span><b>v{format.creativeVersion}</b></div><div><span>Preview</span><b>{previewLabel}</b></div><button onClick={onOpenFigma}>{ru?'Открыть в Figma':'Open in Figma'}<ArrowRight size={12}/></button></div>:<div className="ne-wall-meta"><div className="ne-placement-list">{ps.map(p=><div className="ne-placement-row" key={p.id}><span><b>{p.platform}</b><small>{p.placement}</small></span><em className={statusClass(ttKnown(p)?'ready':'unknown')}>{ttKnown(p)?'TT ✓':'TT ?'}</em></div>)}</div></div>}</article>})}</div>
 </div>;
}
