...
function CreativePreview({format,command,onOpen}:{format:VisualFormat;command:Command;onOpen:()=>void}){
 const frame=useRef<HTMLIFrameElement>(null);const live=Boolean(format.previewUrl);
 useEffect(()=>{if(live)frame.current?.contentWindow?.postMessage(command,'*')},[command,live]);
 if(format.previewUrl)return <div className="bm-live-frame" style={{aspectRatio:`${format.width}/${format.height}`}}><iframe ref={frame} src={format.previewUrl} title={`${format.size} live creative`} sandbox="allow-scripts allow-same-origin"/><button className="bm-live-open" onClick={onOpen} aria-label="Open creative in Figma">↗</button></div>;
...
export default function CampaignWall({campaign,locale,onOpenFigma}:{campaign:Campaign;locale:Locale;onOpenFigma:()=>void}){
 ...
 const liveCount=useMemo(()=>campaign.formats.filter(f=>Boolean(f.previewUrl)).length,[campaign.formats]);
 const maxDuration=useMemo(()=>Math.max(6,...campaign.formats.filter(f=>f.previewUrl).map(f=>Number(f.durationSec)||0)),[campaign.formats]);
