...
export type Campaign={
 id:string;
 name:string;
 status:"draft"|"media-ready"|"creative"|"compliance"|"ready"|"delivered";
...
export function compileVisualFormats(placements:Placement[],existing:VisualFormat[]=[]):VisualFormat[]{
 const existingByKey=new Map(existing.map(f=>[sizeKey(f.width,f.height),f]));
...
export function campaignReadiness(c:Campaign){
 let ready=0,warning=0,blocked=0,unknown=0;
 for(const p of c.placements){const f=c.formats.find(x=>x.placementIds.includes(p.id));const s=placementReadiness(p,f);if(s==="ready")ready++;else if(s==="warning")warning++;else if(s==="blocked")blocked++;else unknown++}
...
export function can(role:AccessRole,action:"edit-creative"|"edit-campaign"|"manage-access"|"deliver"){
 const matrix:Record<AccessRole,Set<string>>={
  owner:new Set(["edit-creative","edit-campaign","manage-access","deliver"]),
  admin:new Set(["edit-creative","edit-campaign","manage-access","deliver"]),
  designer:new Set(["edit-creative"]),
  producer:new Set(["edit-campaign","deliver"]),
  viewer:new Set(),
 };
