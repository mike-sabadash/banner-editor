...
export function compileVisualFormats(placements:Placement[],existing:VisualFormat[]=[]):VisualFormat[]{
 ...
 return [...grouped.entries()].map(([key,ps])=>{const first=ps[0],old=existingByKey.get(key);return {...old,id:old?.id||formatId(first.width,first.height),width:first.width,height:first.height,size:`${first.width}×${first.height}`,placementIds:ps.map(p=>p.id),creativeState:old?.creativeState||"missing",creativeVersion:old?.creativeVersion||0,exportScale:old?.exportScale,maxBytes:old?.maxBytes,exportType:old?.exportType,requirementsSource:old?.requirementsSource,previewUrl:old?.previewUrl,previewHtml:old?.previewHtml,previewSvg:old?.previewSvg,previewType:old?.previewType,durationSec:old?.durationSec,estimatedZipKb:old?.estimatedZipKb,clickTagPresent:old?.clickTagPresent,publishedAt:old?.publishedAt};}).sort((a,b)=>b.width*b.height-a.width*a.height);
}
