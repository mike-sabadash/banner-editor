...
export type AssetQualityResult={assetId:string;formatId:string;sceneId:string;layerId:string;layerName:string;sourceWidth:number;sourceHeight:number;placedWidth:number;placedHeight:number;density:number;requiredScale:ExportScale;state:AssetQualityState};
export const effectiveSourceDensity=(source:Pick<AssetSource,"sourceWidth"|"sourceHeight">,placed:{width:number;height:number})=>{if(!source.sourceWidth||!source.sourceHeight||placed.width<=0||placed.height<=0)return 0;return Math.min(source.sourceWidth/placed.width,source.sourceHeight/placed.height)};
export const densityState=(density:number,required:ExportScale):AssetQualityState=>density+1e-6>=required?"ok":density+1e-6>=required*.75?"warning":"insufficient";
export function assetQualityForCampaign(campaign:Campaign):AssetQualityResult[]{const assets=campaign.creativeDocument?.assets||{},scenes=campaign.creativeDocument?.scenes||[];const results:AssetQualityResult[]=[];for(const format of campaign.formats){const requiredScale=normalizedExportScale(format.exportScale);for(const scene of scenes){for(const layer of scene.layers||[]){const assetId=layer.provenance?.assetId;if(!assetId)continue;const asset=assets[assetId];if(!asset||asset.vector||!asset.sourceWidth||!asset.sourceHeight)continue;const box=layer.masterBox||layer.box;if(!box)continue;const placedWidth=Math.max(0,format.width*Number(box.w||0)/100),placedHeight=Math.max(0,format.height*Number(box.h||0)/100),density=effectiveSourceDensity(asset,{width:placedWidth,height:placedHeight});results.push({assetId,formatId:format.id,sceneId:scene.id,layerId:layer.id,layerName:layer.name||assetId,sourceWidth:asset.sourceWidth,sourceHeight:asset.sourceHeight,placedWidth,placedHeight,density,requiredScale,state:densityState(density,requiredScale)})}}}return results}
