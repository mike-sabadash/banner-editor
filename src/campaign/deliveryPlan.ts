...
async function parseXlsx(file:File){const zip=await unzip(await file.arrayBuffer()),shared=sharedStrings(zip.get("xl/sharedStrings.xml"));const sheets=[...zip.entries()].filter(([name])=>/^xl\/worksheets\/sheet\d+\.xml$/.test(name)).sort();return sheets.map(([name,xml])=>({name,rows:sheetRows(xml,shared)}))}
