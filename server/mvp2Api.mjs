...
export async function handleMvp2Api(req,res,{json,readBody}){
 ...
 try{
  ...
  if(req.method==="POST"&&url==="/api/campaigns"){const body=await readBody(req);return json(req,res,201,await bannermaticStore.createCampaign(auth,body));}
  ...
  if(campaignMatch&&(req.method==="PUT"||req.method==="PATCH")){const body=await readBody(req);return json(req,res,200,await bannermaticStore.updateCampaign(auth,decodeURIComponent(campaignMatch[1]),body));}
