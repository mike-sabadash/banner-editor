...
const ready={id:'c1',name:'Launch',creativeVersion:4,mediaPlanVersion:2,ttSnapshotVersion:3,placements:[placement],formats:[{id:'f1',width:300,height:250,placementIds:['p1'],creativeState:'published',creativeVersion:4,estimatedZipKb:120,durationSec:5,previewUrl:'https://preview/f1',previewType:'html'}]};
describe('MVP2 campaign builds',()=>{
 it('pins all campaign versions in a reproducible manifest',()=>{const b=createCampaignBuildManifest(ready);expect(b.pins).toEqual({creativeVersion:4,mediaPlanVersion:2,ttSnapshotVersion:3});expect(b.placements[0]).toMatchObject({placementId:'p1',formatId:'f1',creativeVersion:4,impressionUrl:'https://tracker/p',clickUrl:'https://click'})});
