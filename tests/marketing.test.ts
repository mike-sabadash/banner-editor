...
describe('Bannermatic public SaaS entry',()=>{
 ...
 it('routes anonymous visitors to marketing before auth',()=>{expect(main).toContain('if(!hasToken()&&auth)');expect(main).toContain('if(!hasToken())return <MarketingHome');expect(main).toContain('?auth=login');expect(main).toContain('?auth=register')});
