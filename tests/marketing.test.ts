...
describe('Bannermatic public SaaS entry',()=>{
 ...
 it('opens Studio auth and the campaign product directly instead of the marketing page',()=>{expect(main).toContain('return <BannermaticProduct/>');expect(main).toContain('return <AuthPage mode={auth}');expect(main).not.toContain('return <MarketingHome');expect(main).not.toContain('bootstrapProjectPersistence');expect(main).not.toContain('installEditorRuntime')});
