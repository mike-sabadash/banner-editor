...
describe('Bannermatic public SaaS entry',()=>{
 ...
 it('routes anonymous visitors to marketing before auth',()=>{expect(main).toContain('!enterApp&&!hasToken()');expect(main).toContain('<MarketingHome');expect(main).toContain('?auth=login');expect(main).toContain('?auth=register')});
